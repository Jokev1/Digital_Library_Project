...
const preprocess = (text) => {
  ...
  const stopWords = new Set([
    "the", "a", "an", "in", "on","at","for","to","of","and","is","are","was","were","be","been","being","have",
    "has","had","do","does","did","will","would","shall","should","can","could","may","might","must","ought",
  ]);
...
const SemanticResult = ({ query }) => {
  ...
  useEffect(() => {
    // init here
    const processedDocs = Object.entries(BookList).map(([key, book]) => ({
    ...
    const allTokens = processedDocs.flatMap((doc) => [
      ...doc.title,
      ...doc.authors,
      ...doc.genres,
      ...doc.description,
  ...
  const handleSearch = (searchQuery) => {
    ...
    const searchResults = Object.entries(BookList).map(([key, book], index) => {
      const docTFIDF = tfidfDocs[index];

      // thresholds for the search
      const titleSimilarity =
        cosineSimilarity(queryTFIDF, docTFIDF.title) * 0.4;
      const authorsSimilarity =
        cosineSimilarity(queryTFIDF, docTFIDF.authors) * 0.1;
      const genresSimilarity =
        cosineSimilarity(queryTFIDF, docTFIDF.genres) * 0.2;
      const descriptionSimilarity =
        cosineSimilarity(queryTFIDF, docTFIDF.description) * 0.3;
      ...
      const authorWords = book.book_authors
        .join(" ")
        .toLowerCase()
        .split(/\s+/);
      const partialMatchScore = titleWords
        .concat(authorWords)
        .reduce((score, word) => {
          return (
            score + (word.startsWith(searchQuery.toLowerCase()) ? 0.05 : 0)
          );
        }, 0);
          ...
          {currentBooks.map(({ key, similarity, book }) => (
            ...
            >
              ...
              <p className="flex flex-row justify-center items-center absolute bottom-2">
                {renderStars(book.book_rating)} {book.book_rating}{" "}
                {kFormatter(book.book_rating_count)}
              </p>
